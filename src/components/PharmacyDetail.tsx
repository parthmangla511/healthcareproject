...
// ===== PHARMACY VIEW DETAILS COMPONENT =====
// This component displays detailed information for the Pharmacy service
// Including available pharmacy records and CRUD operations for records
...
function PharmacyDetail() {
  const [service, setService] = useState<PharmacyService | null>(null);
  const [loading, setLoading] = useState(true);
  const [editId, setEditId] = useState<string | null>(null);
  const [formData, setFormData] = useState({ title: "", description: "" });
  const [records, setRecords] = useState<RecordItem[]>([]);

  // Fetch pharmacy service details
  useEffect(() => {
    fetch("http://localhost:5000/api/services/pharmacy")
      .then((res) => res.json())
      .then((data) => setService(data))
      .catch((error) => console.error("Failed to load pharmacy service:", error))
      .finally(() => setLoading(false));
  }, []);

  // Fetch pharmacy records from MongoDB
  useEffect(() => {
    fetch("http://localhost:5000/api/service-records/pharmacy")
      .then((res) => res.json())
  ...
  const handleDelete = async (id: string) => {
    const confirmed = window.confirm("Delete this pharmacy record?");
    if (!confirmed) return;
  ...
  if (!service) {
    return (
      <div className="demo-container">
        <h2>Pharmacy service not found</h2>
  ...
  return (
    <div className="service-detail-page service-detail-page--pharmacy">
      <div className="service-detail-shell">
        <section className="service-detail-hero">
          <span className="service-detail-badge">💊 Pharmacy</span>
          <h2>{service.title}</h2>
        ...
        <div className="service-detail-grid">
          <section className="service-detail-panel service-detail-form-panel">
            <div className="service-detail-section-title-row">
              <h3>{editId ? "Update Pharmacy Record" : "Add Pharmacy Record"}</h3>
            </div>
            ...
            <form className="service-detail-form" onSubmit={handleSubmit}>
              ...
              />
              <textarea
                placeholder="Details, usage instructions, or description"
          ...
          <section className="service-detail-panel service-detail-records-panel">
            ...
            {records.length === 0 ? (
              <p className="service-detail-empty">No pharmacy records available.</p>
            ) : (
